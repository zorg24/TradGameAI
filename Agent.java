...
//    HashMap<Move, Integer> historyTable = new HashMap<>();

    public void putHistoryTable(Move m, int depth) {
        history[m.from][m.to] = depth;
    }

//    public int getHistoryWeight(Move m) {
//        if (historyTable.containsKey(m))
//            return historyTable.get(m);
//        else
//            return -1;
//    }

    int[][] history = new int[81][81];
    Comparator<Move> moveComparator = Comparator.comparing((Move m) -> history[m.from][m.to]).reversed();
    ...
    private int max2(ChineseCheckersState state, int depth, Move best_move, Alarm timer, int alpha, int beta) {
        ...
        state.getMoves(mov);
//        Collections.sort(mov, moveComparator);
        for (Move m : mov) {
            state.applyMove(m);
            v = Math.max(v, min2(state, depth - 1, junkMove, timer, alpha, beta));
            state.undoMove(m);
            if (v > best) {
                best_move.set(m);
                best = v;
    ...
    private int min2(ChineseCheckersState state, int depth, Move best_move, Alarm timer, int alpha, int beta) {
        nodeVisited++;
        long hash = state.getHash();
...
//        Collections.sort(mov, moveComparator);
        for (Move m : mov) {
            state.applyMove(m);
            v = Math.min(v, max2(state, depth - 1, junkMove, timer, alpha, beta));
            state.undoMove(m);
            if (v > best) {
                best = v;
            }
...
//    }
    ...
    public ChineseCheckersState state = new ChineseCheckersState();
